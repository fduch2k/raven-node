...
module.exports.parseText = function parseText(message, kwargs) {
    kwargs = kwargs || {};
    if (kwargs['params']) {
        kwargs['message'] = util.format(message, kwargs['params']);
        kwargs['sentry.interfaces.Message'] = {
            message: message,
            params: kwargs['params']
        };
        kwargs['checksum'] = utils.constructChecksum(kwargs['sentry.interfaces.Message']);
...
module.exports.parseUser = function parseUser(user, kwargs) {
    kwargs = kwargs || {};
    if (user)
        kwargs['sentry.interfaces.User'] = user;
    return kwargs;
};
